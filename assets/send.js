...
(function(){
  ...
  M('en',{ send_no_balance_xlm:'Not enough spendable XLM — after the network fee and the account reserve you can send about {s} XLM.', rcpt_onchain:'Verified on-chain via Horizon', rcpt_faucet:'Your account had no balance, so it was activated with free test XLM from the Stellar Testnet faucet (Friendbot). This is test money, not real funds.', rcpt_ledger:'Ledger', chk_src:'The signed transaction source does not match your connected wallet.', chk_ops:'The signed transaction must contain exactly one operation.', chk_op:'The signed transaction failed safety verification.', chk_dest:'The signed recipient does not match what you entered.', chk_amt:'The signed amount does not match what you entered.', chk_asset:'The signed asset does not match what you selected.', chk_sig:'The signed transaction carries no signature.', chk_onchain:'On-chain confirmation failed — the network did not record this transaction as successful.' });
  M('ru',{ send_no_balance_xlm:'Недостаточно XLM — после комиссии сети и резерва аккаунта вы можете отправить около {s} XLM.', rcpt_onchain:'Подтверждено в сети через Horizon', rcpt_faucet:'На вашем аккаунте не было баланса, поэтому он был активирован бесплатными тестовыми XLM из крана Stellar Testnet (Friendbot). Это тестовые средства, а не настоящие деньги.', rcpt_ledger:'Леджер', chk_src:'Источник подписанной транзакции не совпадает с подключённым кошельком.', chk_ops:'Подписанная транзакция должна содержать ровно одну операцию.', chk_op:'Подписанная транзакция не прошла проверку безопасности.', chk_dest:'Получатель в подписанной транзакции не совпадает с введённым.', chk_amt:'Сумма в подписанной транзакции не совпадает с введённой.', chk_asset:'Актив в подписанной транзакции не совпадает с выбранным.', chk_sig:'Подписанная транзакция не содержит подписи.', chk_onchain:'Не удалось подтвердить транзакцию в сети — сеть не зафиксировала её как успешную.' });
})();
