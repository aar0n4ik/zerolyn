...
/* ---- i18n (augments app.js dictionaries; full es/de/uk translations) ---- */
(function(){ if(!window.I18N) return; function M(l,o){ window.I18N[l]=Object.assign(window.I18N[l]||{},o); }
  ...
  var ES={st_eyebrow:'ZK Proof Studio',st_compose_h:'Compón la afirmación',st_priv_note:'Los datos privados nunca salen de tu navegador — solo se revelan la prueba, el límite público y el importe pagado.',st_amount:'Importe',st_balance:'Saldo',st_limit:'Límite de cumplimiento',st_private:'privado',st_public:'público',st_c_pos:'positividad',st_c_solv:'solvencia',st_c_comp:'cumplimiento',st_ok:'✓ La afirmación es satisfacible — se puede generar una prueba de conocimiento cero válida.',st_bad:'✗ La afirmación viola una restricción — no puede existir una prueba honesta.',st_run:'Generar prueba y verificar on-chain',st_run_busy:'Probando en tu navegador…',st_vis_h:'Prueba Groth16',st_vis_empty:'Mueve los controles para construir una afirmación y luego genera una prueba real para ver aquí sus elementos de grupo y la calldata on-chain.',st_proving:'Generando witness y prueba Groth16…',st_pub_h:'Señales públicas',st_calldata_h:'Calldata on-chain',st_copy:'Copiar',st_copied:'Copiado',st_time:'Tiempo de prueba',st_onchain_h:'Verificación on-chain',st_onchain_lead:'La prueba anterior la verifica un contrato Soroban en Stellar Testnet — el emparejamiento BLS12-381 se ejecuta dentro del host de Stellar, no en esta página.',st_adv:'Avanzado · JSON de prueba en bruto'};
  var DE={st_eyebrow:'ZK Proof Studio',st_compose_h:'Aussage zusammenstellen',st_priv_note:'Private Eingaben verlassen nie deinen Browser — nur der Beweis, das öffentliche Limit und der gezahlte Betrag werden offengelegt.',st_amount:'Betrag',st_balance:'Guthaben',st_limit:'Compliance-Limit',st_private:'privat',st_public:'öffentlich',st_c_pos:'Positivität',st_c_solv:'Solvenz',st_c_comp:'Compliance',st_ok:'✓ Die Aussage ist erfüllbar — ein gültiger Zero-Knowledge-Beweis kann erzeugt werden.',st_bad:'✗ Die Aussage verletzt eine Constraint — es kann kein ehrlicher Beweis existieren.',st_run:'Beweis erzeugen & on-chain prüfen',st_run_busy:'Beweise in deinem Browser…',st_vis_h:'Groth16-Beweis',st_vis_empty:'Bewege die Regler, um eine Aussage zu bauen, und erzeuge dann einen echten Beweis, um hier seine Gruppenelemente und On-Chain-Calldata zu sehen.',st_proving:'Erzeuge Witness und Groth16-Beweis…',st_pub_h:'Öffentliche Signale',st_calldata_h:'On-Chain-Calldata',st_copy:'Kopieren',st_copied:'Kopiert',st_time:'Beweiszeit',st_onchain_h:'On-Chain-Verifikation',st_onchain_lead:'Der obige Beweis wird von einem Soroban-Vertrag im Stellar Testnet verifiziert — das BLS12-381-Pairing läuft im Host von Stellar, nicht auf dieser Seite.',st_adv:'Erweitert · rohes Proof-JSON'};
  var UK={st_eyebrow:'ZK Proof Studio',st_compose_h:'Складіть твердження',st_priv_note:'Приватні дані ніколи не залишають ваш браузер — розкриваються лише доказ, публічний ліміт і сплачена сума.',st_amount:'Сума',st_balance:'Баланс',st_limit:'Комплаєнс-ліміт',st_private:'приватно',st_public:'публічно',st_c_pos:'позитивність',st_c_solv:'платоспроможність',st_c_comp:'комплаєнс',st_ok:'✓ Твердження здійсненне — можна згенерувати коректний zero-knowledge доказ.',st_bad:'✗ Твердження порушує обмеження — чесного доказу не існує.',st_run:'Згенерувати доказ і перевірити ончейн',st_run_busy:'Доводжу у вашому браузері…',st_vis_h:'Доказ Groth16',st_vis_empty:'Рухайте повзунки, щоб скласти твердження, потім згенеруйте справжній доказ, щоб побачити тут його елементи групи та ончейн-calldata.',st_proving:'Генерую witness і доказ Groth16…',st_pub_h:'Публічні сигнали',st_calldata_h:'Ончейн-calldata',st_copy:'Копіювати',st_copied:'Скопійовано',st_time:'Час доказу',st_onchain_h:'Ончейн-перевірка',st_onchain_lead:'Доказ вище перевіряє контракт Soroban у Stellar Testnet — спарювання BLS12-381 виконується в хості Stellar, а не на цій сторінці.',st_adv:'Розширено · сирий proof JSON'};
  M('en',EN); M('ru',RU); M('es',ES); M('de',DE); M('uk',UK);
})();
...
async function run(){
  ...
  try{
    ...
    var msg=art?t('ver_no_artifacts'):em.slice(0,200);
    if(vis){ vis.innerHTML=''; vis.appendChild(el('div','st-vis-empty',msg)); }
    if(window.SP_toast) window.SP_toast(msg,'err');
